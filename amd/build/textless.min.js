...
define('local_textless_forum/textless', ['core/templates', 'core/str'], function(Templates, Str) {
    ...
    Recorder.prototype.registerListeners = function() {
        ...
        if (this.startRecordingBtn) {
            this.startRecordingBtn.addEventListener('click', function() {
                self.beginRecording();
            });
        }
        if (this.switchCameraBtn) {
            this.switchCameraBtn.addEventListener('click', function() {
                self.switchCamera();
            });
        }
        if (this.cancelPreviewBtn) {
            this.cancelPreviewBtn.addEventListener('click', function() {
                self.cancelPreview();
            });
        }
     ...
     * recording" separately so they can frame themselves before the
     * recording (and its time limit) begins.
     *
     * @param {String} type Either 'audio' or 'video'.
     * @returns {Promise} Resolves once the stream has been acquired (and, for
    ...
    Recorder.prototype.start = function(type) {
        ...
        return navigator.mediaDevices.getUserMedia(constraints).then(function(stream) {
            ...
            if (type === 'video') {
                var detected = self.config.allowswitchcamera ? self.detectCameras() : Promise.resolve();
                return detected.then(function() {
                    self.preview.srcObject = stream;
                    self.previewWrapper.classList.remove('d-none');
                    self.preview.play().catch(function() {
                        return;
                    });
                    self.setButtons('previewing');
     ...
     * use, so switchCamera() can offer the user the next one.
     *
     ...
     * getUserMedia() call.
     *
     * @returns {Promise} Resolves once the device list has been refreshed.
     */
    Recorder.prototype.detectCameras = function() {
        var self = this;
        this.videoDevices = [];
        this.deviceIndex = -1;
        ...
        return navigator.mediaDevices.enumerateDevices().then(function(devices) {
            self.videoDevices = devices.filter(function(device) {
                return device.kind === 'videoinput';
            });

            var track = self.stream && self.stream.getVideoTracks()[0];
            var currentId = track && track.getSettings && track.getSettings().deviceId;
            var matchedIndex = currentId ? self.videoDevices.findIndex(function(device) {
                return device.deviceId === currentId;
            }) : -1;
            self.deviceIndex = matchedIndex !== -1 ? matchedIndex : 0;
            return;
        }).catch(function() {
            self.videoDevices = [];
            self.deviceIndex = -1;
     ...
     *
     * @returns {Promise} Resolves once the preview is showing the new camera.
     */
    Recorder.prototype.switchCamera = function() {
        var self = this;
        if (this.videoDevices.length < 2) {
            return Promise.resolve();
        }

        var nextIndex = (this.deviceIndex + 1) % this.videoDevices.length;
        var deviceId = this.videoDevices[nextIndex].deviceId;
        ...
        return navigator.mediaDevices.getUserMedia({
            ...
            var oldTrack = self.stream.getVideoTracks()[0];
            if (oldTrack) {
                self.stream.removeTrack(oldTrack);
                oldTrack.stop();
            }
            self.stream.addTrack(newTrack);

            self.preview.srcObject = self.stream;
            self.preview.play().catch(function() {
                return;
            });

            self.deviceIndex = nextIndex;
            return;
        }).catch(function() {
            self.setStatus('errornopermission');
        });
     ...
     */
    Recorder.prototype.cancelPreview = function() {
        this.stopStream();
